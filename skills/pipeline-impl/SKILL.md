...
0. **Preflight** — run `bash <this skill's base dir>/../pipeline-preflight/scripts/preflight.sh --stage impl` (+ the five envelope fields
   verbatim when one is present). `0` ⇒ step 1's pull + `current.json` read and step 2's slot resolve + install check are DONE — use its
   printed lines (card picking in step 1 and the `feat/<feature>` create/reconcile in step 2 stay yours); `3` ⇒ verify the slot skill
   installed yourself; other non-zero ⇒ STOP with its printed reason; script absent ⇒ run the steps as written (CONTRACT §shim loop).
