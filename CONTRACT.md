...
`coordinate.sh` ships only the read-only `doctor`/`status` preflight). EITHER coordinator observes remote Git as the only business truth, routes on the journal
